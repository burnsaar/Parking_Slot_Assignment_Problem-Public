...
def piece(x, A_rate, D_rate, D_rate_dbl_park, df, Max_Q, len_scenario):
    ...
    if queue_buildup_rate < 0:
        ...
        func = np.piecewise(x, condlist, funclist)
    ...
    if len(df) == 1:
        ...
        return func, sum(queue_duration), t_queue_remove
...
for c in [1,2,4,7]: #added
    ...
    #for i in range(0,1): #added
        ...
        if df.empty == True:
            ...
            #upper bounds?  maybe just super large since it doesn't matter?
            #The upper bounds on the integration could be more specific, but the
            #piece function is defined such that exceeding the last double parking
            #event dissapation isn't going to result in extra area between the curves,
            #so set the upper bound to something very large that will never be exceeded
            [total_veh_delay, error] = integrate.quad(lambda x: x*A_rate - piece(x, A_rate, D_rate, D_rate_dbl_park, df, Max_Q, len_scenario)[0], 0, len_scenario, limit = 1000, epsrel = .01)
            total_veh_delay_inst_FCFS.iloc[i][c].append(total_veh_delay)
            total_veh_delay_error_inst_FCFS.iloc[i][c].append(error)
            
            [func, queue_duration, t_queue_remove] = piece(x, A_rate, D_rate, D_rate_dbl_park, df, Max_Q, len_scenario)
        ...
        if df.empty == True:
            ...
            #event dissapation isn't going to result in extra area between the curves,
            #so set the upper bound to something very large that will never be exceeded
            [total_veh_delay, error] = integrate.quad(lambda x: x*A_rate - piece(x, A_rate, D_rate, D_rate_dbl_park, df, Max_Q, len_scenario)[0], 0, len_scenario, limit = 1000, epsrel = .01) #, epsabs = 0.01, epsrel = 0.1, limit = 100
            total_veh_delay_inst_phi5.iloc[i][c].append(total_veh_delay)
            total_veh_delay_error_inst_phi5.iloc[i][c].append(error)
...
# import pickle
# with open('pub_Pitt_queuing_data_post_process_phi5_limit30_23_Mar_2023_buffer_5_basecase.pkl', 'wb') as file:
